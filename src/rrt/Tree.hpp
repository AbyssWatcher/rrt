...
public:
    ...
    void getPath(std::function<void(const T& stateI)> callback,
        ...
        if (reverse) {
            ...
            // collect states in list in reverse order
            ...
            // then pass them one-by-one to the callback, reversing the order so
    ...
    /**
     * Get the path from the receiver's root point to the dest point. Similar to
     * the above method, but appends the points to an output vector rather than
     * calling a user-provided callback for each one.
     *
    ...
    /**
     * This method is the same as the above, but returns the path vector rather
     * than appending to a vector passed as a parameter.
     */
